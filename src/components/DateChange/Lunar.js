...
let Lunar = {
    ...
    Date:function(type){//'yyyy-mm-dd'
        ...
        if(temp.split("m").length!=3){//处理mmm或者m的情况
            temp = temp.replace(/m{1,}/,(date.getMonth()+1).toString().split('').reverse().join(''))
        }else{
            temp = temp.replace(/m{1,}/,month.toString().split('').reverse().join(''))
        }
        if(temp.split("d").length!=3){//处理DDD或者D的情况
            temp = temp.replace(/d{1,}/,date.getDate().toString().split('').reverse().join(''))
        }else{
            temp = temp.replace(/d{1,}/,day.toString().split('').reverse().join(''))
        }
